...
%This code constructs two screens, one with a given surface
...
%function(s)) before computing it, by feeding it to an integrator class.
%Although they are not related in the child/parent sense of OO, their
%relationship is more of an unprogrammable ying/yang type... they should be
%designed to work together, and changes to one will affect the other.
...
%but uses more sophisticated Generalised Gauss + Duffy routines for
%singulariti
%This has made debugging it really easy, and is actually quite neat.es. This approach is designed so that the 'integrator' can be
%changed relatively easily, without having to change other components of
%the code. For example, Steve can add Filon, and Daan can add Steepest
%descent.
%The 'BEMintegral2D' class automatically decomposes its self into
%subintegrals of the same class, which are all either smooth, singular at
%the corner, or singular on the diagonal of a square.
...
addpath ..; % add  everything in the folder up, where the files live
addpath ../CompGaussFiles; %files for composite Gauss, from PhD
%user defined parameters:
%-----------------------------------------------------------------------%
kwave=40; %wavenumber
pMax=4; nLayers=2*pMax; sigmaGrad=0.15; %basis parameters
NquadPointsPerWavelength=12; %number of points per wavelength for comp Gauss
%define the screens
vertices1=[0 0       %first vertex
          2*pi 0];     %second vertex
      
vertices2=[0 -pi       %first vertex
          2*pi -pi];     %second vertex
 ...
 %DOWNWARDS (not sure why this is, but it's sort of arbitrary for the
 %screen...)
...
g=density(Screen2,@(s) abs(pi-s).^2); %\varphi in our notation
...
for n=1:basEls
    ...
    %would be even cooler if I could write 'L2(f-S*beamGOA,V.el(n))',
    %although haven't defined addition for operation
    %BoundaryFunction+BoundaryIntegral, so can't compute f-S*beamGOA.
end
...
%like a function (note that evaluation depends on quad routine)
