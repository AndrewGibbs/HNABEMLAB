...
JST.example_card = _.template(
    '<div class="card_container explorer_view add_long_title" data-ui-component="card">' +
        '<a href="<%= url %>">' +
        '<div class="card_media" style="background-image:url(<%= usePlatformScreenshotsForCards(thumbnail_url) %>);">' +
        '<img alt="<%= title %>" src="<%= usePlatformScreenshotsForCards(thumbnail_url) %>" />' +
        '</div>' +
        '<div class="card_body">' +
        ...
        $.getJSON(productJsonUrl, function (productData) {
                ...
                $.each(productData, function(index, example) {
                    if ($.inArray(example.open_command, jsonData.examples) >= 0 ) {
                        example.url = example.url.replace(/^\.\.\//, '');
...
function usePlatformScreenshotsForCards(src) {
    "use strict";
    return src;
}
